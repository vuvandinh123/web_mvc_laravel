...
<div class="container-fluid py-4">
    ...
    <a href="{{ route('post.index') }}" class="fs-6">Quay lại</a>
        <div class="">
            <h4 class="text-center mb-4">Cập nhật bài viết</h4>
            
        </div>
        <form action="{{ route('post.update',['post'=>$post->id]) }}" id="create" method="post"enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <input
                        placeholder=""
                        style="min-height: 40px" class="form-control" name="metades" id="metades"
                        value="{{ $post->metadesc }}"
                        >
                    <span class="text-danger alert" style="font-size: 12px"></span>
                </div>
...
<script>
    $(document).ready(function() {
            validateInput([ '#title','#topic_id', '#metakey','#metades','#detail','#type'], 'Vui lòng nhập trường này');
            $('#create').submit(function(e) {
                let fieldIds = ['title','topic_id', 'metakey', 'metades','detail','type']
